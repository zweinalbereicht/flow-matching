...
def main(args: ScriptArguments) -> None:
    ...
    set_seed(args.seed)
    output_dir = args.output_dir / "cfm" / f"unbalanced_2_kappagmm_{args.dim}_{args.kappa}"
    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"Using device: {device}")
    print(f"GMM Dataset: {args.dim}, {args.kappa}")

    # set some weights here


    weights = torch.tensor([1/2,1/8,1/4,1/16])
    dataset = DatasetkappaGMM(dim=args.dim,device=device, kappa=args.kappa, weights=weights)
    ...
    # save the mus and weights
    torch.save(dataset.mus, output_dir / "mus.pt")
    torch.save(dataset.weights, output_dir / "weights.pt")
